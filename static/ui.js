(() => {
    ...
    const navItem=(label,href,key)=>{const link=document.createElement("a");link.className="app-nav-link";link.href=href;link.innerHTML=`<span aria-hidden="true">${icon(key)}</span><span>${label}</span>`;const exact=location.pathname===href,section=href.includes("#")&&location.pathname==="/dashboard"&&location.hash===href.slice(href.indexOf("#"));if(exact||section)link.setAttribute("aria-current","page");return link;};
