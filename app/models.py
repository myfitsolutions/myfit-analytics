...
class AutomationsDelivery(Base):
    ...
    id = Column(Integer, primary_key=True)
    analytics_studio_id = Column(Integer, ForeignKey("studios.id", ondelete="CASCADE"), nullable=False, index=True)
    integration_id = Column(Integer, ForeignKey("automations_integrations.id", ondelete="CASCADE"), nullable=True, index=True)
    automations_studio_id = Column(String(36), nullable=False)
...
class AutomationsDeliveryAttempt(Base):
    __tablename__ = "automations_delivery_attempts"
    __table_args__ = (Index("ix_automations_delivery_attempts_delivery_created", "delivery_id", "created_at"),)

    id = Column(Integer, primary_key=True)
    delivery_id = Column(Integer, ForeignKey("automations_deliveries.id", ondelete="CASCADE"), nullable=False, index=True)
