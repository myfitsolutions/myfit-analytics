...
STATEMENTS = [
"ALTER TABLE automations_integrations ADD COLUMN IF NOT EXISTS target_revision INTEGER NOT NULL DEFAULT 1",
...
"ALTER TABLE automations_deliveries ADD COLUMN IF NOT EXISTS claim_token VARCHAR(36) NULL",
"ALTER TABLE automations_deliveries ADD COLUMN IF NOT EXISTS claim_expires_at TIMESTAMPTZ NULL",
...
"""UPDATE automations_deliveries SET delivery_status=CASE WHEN normalized_payload IS NULL THEN 'failed_legacy' ELSE 'failed' END, safe_error_code=CASE WHEN normalized_payload IS NULL THEN 'legacy_payload_unavailable' ELSE 'delivery_interrupted' END, claim_token=NULL, claim_expires_at=NULL, next_retry_at=NULL, updated_at=NOW() WHERE delivery_status='delivering' AND claim_expires_at IS NULL""",
"""UPDATE automations_deliveries SET delivery_status='failed', safe_error_code='attempt_exhausted', claim_token=NULL, claim_expires_at=NULL, next_retry_at=NULL, updated_at=NOW() WHERE delivery_status IN ('pending','delivering') AND attempt_count>=5""",
