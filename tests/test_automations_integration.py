...
from app.models import AutomationsDelivery, AutomationsIntegration, Member, Payment, Studio
...
def test_delivery_headers_idempotency_timeout_observability_and_no_secret_persistence(monkeypatch):
    ...
    delivery=AutomationsClient(httpx.MockTransport(handler)).deliver(db,integration,fact,"corr-sent")
    ...
    assert delivery.delivery_status=="accepted" and delivery.evaluation_id=="evaluation-1" and delivery.runs_created==1
    assert "top-secret-token" not in repr(delivery.__dict__)
    repeated=AutomationsClient(httpx.MockTransport(handler)).deliver(db,integration,fact,"corr-repeat")
    assert repeated.id==delivery.id and repeated.attempt_count==2
